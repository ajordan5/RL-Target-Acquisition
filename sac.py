...
class ActorNetwork(nn.Module):
    def __init__(self, input_shape, output_shape, n_features, state_shape, grid_shape, **kwargs):
        ...
        # Final processAgent.load('saved_models_grid/best_undiscounted')
        nn.init.xavier_uniform_(self._h1.weight,
                                gain=nn.init.calculate_gain('relu'))
        nn.init.xavier_uniform_(self._h2.weight,
                                gain=nn.init.calculate_gain('relu'))
...
class CriticNetworkEnemy(nn.Module):
...
class ActorNetworkEnemy(nn.Module):
    def __init__(self, input_shape, output_shape, n_features, **kwargs):
        super(ActorNetworkEnemy, self).__init__()
...
def experiment(alg, mdp, n_epochs, n_steps, n_steps_test):
    ...
    actor_mu_params = dict(network=ActorNetworkEnemy,
                           n_features=n_features,
                           input_shape=actor_input_shape,
                           output_shape=mdp.info.action_space.shape,
                           use_cuda=use_cuda)
    actor_sigma_params = dict(network=ActorNetworkEnemy,
                              n_features=n_features,
                              input_shape=actor_input_shape,
                              output_shape=mdp.info.action_space.shape,
                              use_cuda=use_cuda)
    ...
    critic_params = dict(network=CriticNetworkEnemy,
...
if __name__ == '__main__':
    ...
    # agent = Agent.load('saved_models/model3')
    # core = Core(agent, mdp)
